"""
sindy_eval_utils.py — Κοινά helpers για όλα τα SINDy eval/fusion scripts.
...
ΤΙ ΠΡΟΣΦΕΡΕΙ (ώστε να ΜΗΝ επαναλαμβάνεται κώδικας στα test_sindy_* / fusion_*):
  ...
  * encoded_measurements / seed_context : per-frame encoded z[:4] ανά window — «μέτρηση» για το
...
ΣΗΜ. (run convention): τρέχει ως module που γίνεται import από τα standalone SINDy scripts
(`!python3 cartpole/<file>.py`). Είναι flat στο cartpole/ μαζί με τα vae/lstm/loader, οπότε τα
imports γίνονται απευθείας (το `from sindy_core import list_npz` κάνει και path-bootstrap). Τα
torch/vae/lstm/loader imports μένουν LAZY (μέσα στις συναρτήσεις) ώστε οι καθαρά-numpy helpers
(encoded_measurements, seed_context, sq_err_standardized, ...) να δουλεύουν ΧΩΡΙΣ torch.
"""
import os

import numpy as np

from sindy_core import list_npz   # + path-bootstrap: κάνει importable τα vae/lstm/loader του cartpole/


# ---------------------------------------------------------------------------
# Σταθερές (ίδιες με τα test_pX του baseline)
# ---------------------------------------------------------------------------
LATENT_SIZE = 64
N_SUP = 4
N_ACTIONS = 2
...
def _free_run(model, batch, seed_phys=None):
    """seed_phys: (B,4) standardized override για τις dims[:4] του seed (None -> ως έχει)."""
    import torch
...
def lstm_free_run_dir(lstm, latent_dir, mean, std, device,
    ...
    """ENCODED free-running rollout πάνω σε latent_dir (ΙΔΙΑ windows με assemble_windows).
       ...
       seed_phys_std (N,4) -> standardized override των seed dims[:4] (για το seed-denoise).
...
# ---------------------------------------------------------------------------
# Per-frame encoded z[:4] ανά window — numpy-only (μέτρηση για Kalman / seed-denoise)
...
def seed_context(latent_dir, mean, std, seq_len=SEQ_LEN, stride=TEST_STRIDE):
    """Per-window context του seed για το SINDy seed-denoising (F). Αλιγναρισμένο ΘΕΣΗ-ΘΕΣΗ με
       sindy_core.assemble_windows. -> dict:
         ...
         prev_raw     (N,4): de-standardized encoded z[s-1,:4] (=enc_seed αν s==0)
...
def encoded_measurements(latent_dir, mean, std, seq_len=SEQ_LEN, stride=TEST_STRIDE):
    """ -> (meas_raw (N,L,4), seed_raw (N,4)) σε RAW physical units.
       meas_raw[w,k] = de-standardized encoded z[:4] στο frame s+k+1  (k=0..L-1)
       seed_raw[w]   = de-standardized encoded z[:4] στο seed frame s.
...
# ---------------------------------------------------------------------------
# Μετρική: standardized squared error (N,L,4) — κοινή σύμβαση για ΟΛΕΣ τις μεθόδους
# ---------------------------------------------------------------------------
def sq_err_standardized(pred_raw, gt_raw, std):
    """RAW preds/gt -> (N,L,4) standardized squared error (όπως test_pX)."""
    std4 = np.asarray(std[:N_SUP], np.float64)
