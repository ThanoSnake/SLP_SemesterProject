    ...
    \begin{columns}[onlytextwidth, T]
        \begin{column}{0.5\textwidth}
            \begin{block}{Το Παράδοξο της Αρχής 3}
                \begin{itemize}
                    \item \scriptsize Υπό την παρουσία χωρικού θορύβου, το μοντέλο με ασθενή επίβλεψη (\textlatin{P3-weak}) \textbf{υπερτερεί} του πλήρως επιβλεπόμενου \textlatin{Baseline}!
                    \item \scriptsize \textbf{Εξήγηση:} Η εκπαίδευση με «θορυβώδεις» εκτιμήσεις ταχυτήτων (μέσω πεπερασμένων διαφορών) λειτούργησε ως έμμεση κανονικοποίηση (\textlatin{implicit regularizer}).
                    \item \scriptsize Απέτρεψε τον \textlatin{encoder} από το να κάνει \textlatin{overfit} στις τέλειες, καθαρές λεπτομέρειες των δεδομένων εκπαίδευσης, αυξάνοντας τη γενίκευση σε \textlatin{noisy OOD} καταστάσεις.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.46\textwidth}
            ...
            \begin{block}{Συμπέρασμα}
                \scriptsize Η απαίτηση για τέλεια δεδομένα επίβλεψης είναι μύθος. Η προσεγγιστική, ασθενής επίβλεψη φυσικών μεγεθών όχι μόνο επαρκεί, αλλά καθιστά το μοντέλο πιο εύρωστο σε συνθήκες εκτός κατανομής.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[onlytextwidth, T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Μελλοντικές Επεκτάσεις}
                \begin{itemize}
                    \item \scriptsize \textbf{Φυσικοί Κανόνες από LLMs:} Χρήση μεγάλων γλωσσικών μοντέλων για τον αυτόματο εντοπισμό και ορισμό κανόνων equivariance/invariance.
                    \item \scriptsize \textbf{Σύνδεση με Έλεγχο:} Πραγματική ενσωμάτωση του ερμηνεύσιμου $z_{\text{phys}}$ σε ελεγκτές \textlatin{MPC} για ασφαλές \textlatin{planning}.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Κατανομή Εργασιών \& Ρόλοι Μελών}
    ...
    \begin{tabular}{l l p{7.5cm}}
        ...
        \textbf{Ρήγας Αθανάσιος} & 03122078 & VAE/LSTM updates, Scheduled Sampling, Curriculum Learning. \\
