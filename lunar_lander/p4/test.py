...
@torch.no_grad()
def ssim_batch(a, b, window):
    ch = a.size(1); pad = window.size(-1) // 2
...
def model_recon(model, x):
    return model(x)[0]          # baseline -> recon ; P4 -> composite


@torch.no_grad()
def evaluate(model, loader, device):
    model.eval()
    win = None; se, ssim_sum, n = 0.0, 0.0, 0
...
def decoder_param_count(model):
    if hasattr(model, "dec_lander"):     # VAE_P4
        return count_params(model.dec_lander, model.dec_flags, model.dec_bg)
    parts = []
...
if __name__ == "__main__":
    ...
    b_tot, p_tot = count_params(base), count_params(p4)
