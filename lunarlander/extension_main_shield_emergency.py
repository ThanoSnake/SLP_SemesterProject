"""
extension_main_shield_emergency.py — Extension 4: the world model as an ADDITIVE vertical shield.
...
SAVE_DIR = outputs("lunarlander_ext4_main_shield_emergency")
...
EMERGENCY_Y_MAX = 1.00            # looser: give main time before the ground
EMERGENCY_VY_MAX = -0.20          # looser: catch the dangerous descent earlier
EMERGENCY_COST_MARGIN = 0.01      # a smaller required gain, since we already have hard gates
...
def run_episode(controller, env, vae, lstm, mean_t, std_t, device, ep_seed, record=False):
    ...
    for _ in range(MAX_STEPS):
        ...
        elif controller == "emergency_shield":
...
def main():
    ...
    plt.ylabel("episode return"); plt.title(f"Emergency main-shield — return (model={MODEL}, wind={ENABLE_WIND})")
    plt.grid(alpha=0.3, axis="y"); plt.legend(); plt.tight_layout()
    p = os.path.join(SAVE_DIR, f"ems_{MODEL}_returns.png")
    plt.savefig(p, dpi=150, bbox_inches="tight"); plt.close(); print("saved:", p)

    np.savez(os.path.join(SAVE_DIR, f"ems_{MODEL}_results.npz"),
