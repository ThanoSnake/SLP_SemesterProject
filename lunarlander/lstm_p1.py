...
class VAE_P1(nn.Module):
    ...
    def __init__(self, n_sup=8, n_img=56, frame_channels=3, out_channels=3):
        super().__init__()
        self.n_sup = n_sup; self.n_img = n_img
        self.frame_channels = frame_channels
        self.latent_size = n_sup + n_img
        feat = 64 * 10 * 15
        self.state_encoder = self._conv_stack(2 * frame_channels)
        self.image_encoder = self._conv_stack(frame_channels)
    ...
    def encode(self, x):
        hs = self.state_encoder(x).flatten(1)
        hi = self.image_encoder(x[:, :self.frame_channels]).flatten(1)
