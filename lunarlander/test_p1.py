...
# ---------------------------------------------------------------------------
# Model definitions (ΙΔΙΑ με lunar_eval_baseline_vs_p1.py)
# ---------------------------------------------------------------------------
MODELS = [
    ...
    {"label": "Principle 1", "color": "C1",
     ...
     "vae_ckpt": "/kaggle/input/datasets/iliasbakos/lunar-vae-p1/vae_p1_best.pth",
     "lstm_ckpt": {
         "hybrid":  "/kaggle/input/datasets/iliasbakos/lunar-lstm-p1/lstm_p1_best.pth",
...
def evaluate_model_noisy(m, device, mean_s, std_s, noise_level, noise_type):
    """Load VAE, encode test images WITH noise, evaluate LSTM -> (N,L,8) per mode."""
...
def main():
    ...
    # all_err[noise_level][label][mode] = (N, L, 8)
    all_err = {}
    for nl in NOISE_LEVELS:
        print(f"\n{'='*60}")
