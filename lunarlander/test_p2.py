...
# ---------------------------------------------------------------------------
# Photometric transforms — float [0,1] image tensors (B/T,3,H,W)
# ---------------------------------------------------------------------------
...
def plot_perdim(err, save_dir):
    """(2) Per-dim median+IQR state-MSE (2×4) — Baseline vs P2."""
    horizons = np.arange(1, SEQ_LEN + 1)
    ...
    for d in range(N_SUP):
        ...
        if d == 0:
            ax.legend()
    plt.suptitle(f"Per-dim state-MSE (encoded) | {TRANSFORM_TYPE} level={TRANSFORM_LEVEL:.2f}", y=1.01)
    plt.tight_layout()
...
def plot_trajectory(data, mean_s, std_s, save_dir, rng):
    ...
    for wi in range(N_TRAJ_WINDOWS):
        ...
        p_phys = data[p2]["pred"][w] * std8 + mean8
...
# ---------------------------------------------------------------------------
# (5) Frame-encoding invariance check (8 dims -> gridspec 2×8)
# ---------------------------------------------------------------------------
@torch.no_grad()
def plot_frame_encoding(mean_s, std_s, device, save_dir, rng):
    ...
    for m in MODELS:
        ...
        preds_clean[m["label"]] = mu_c[0, :N_SUP].cpu().numpy() * std8 + mean8
        preds_tf[m["label"]] = mu_t[0, :N_SUP].cpu().numpy() * std8 + mean8
        del vae
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    gs = fig.add_gridspec(2, 8, height_ratios=[1.3, 1.0], hspace=0.45, wspace=0.4)
    ax0 = fig.add_subplot(gs[0, 0:4]); ax0.imshow(orig_np); ax0.set_title("original frame_t"); ax0.axis("off")
    ax1 = fig.add_subplot(gs[0, 4:8]); ax1.imshow(tf_np)
    ...
    for d in range(N_SUP):
        ...
        ax.axhline(gt[d], color="k", ls=":", lw=1)
        ax.axhline(0, color="0.6", lw=0.6)
        ax.set_xticks([0, 1, 2]); ax.set_xticklabels(["GT", base[:4], p2[:4]], rotation=20, fontsize=7)
        eb = preds_tf[base][d] - gt[d]; ep2 = preds_tf[p2][d] - gt[d]
        ax.set_title(f"{DIM_LABELS[d]}\nΔb={eb:+.2f} Δp={ep2:+.2f}", fontsize=7)
    ...
    for d in range(N_SUP):
        ...
        sb = abs(preds_tf[base][d] - preds_clean[base][d])
