"""
test_sindy_standalone.py — Standalone αξιολόγηση του SINDy world-model (env-agnostic: CartPole/LunarLander).
...
ENV-AGNOSTIC: ΙΔΙΟ αρχείο για cartpole & lunarlander· το environment (N_SUP, physics, placeholders)
ορίζεται από τα sindy_core/sindy_eval_utils του φακέλου. Τρέξε: !python3 <env-folder>/test_sindy_standalone.py
...
# ---------------------------------------------------------------------------
# CONFIG — env-agnostic· όλο το per-environment config (DATA_ROOT/CKPTs/N_SUP/DIM_*/FEATURE_MODE/
# NOISE_CONDS/THRESHOLD/...) έρχεται από το sindy_eval_utils & sindy_core (import * πιο πάνω).
# ---------------------------------------------------------------------------
SAVE_DIR = f"/kaggle/working/sindy_{ENV_TAG}_standalone_out"
LOG_Y = True
